...
public class UnicodeEscape {
	...
	/**
	 * Unescapes an escaped UTF-8 sequence from a {@link MemorySegment},
	 * producing a {@link ByteString} without allocating an intermediate
	 * {@link String}.
	 *
	 * @param segment UTF-8 bytes
	 * @param start   start index (inclusive)
	 * @param end     end index (exclusive)
	 * @return unescaped UTF-8 {@link ByteString}
	 */
	public static ByteString unescapeByteString(MemorySegment segment, long start, long end) {
		if (start >= end) {
			return ByteString.empty();
		}

		long backSlashIdx = indexOf(segment, start, end, (byte) '\\');
		if (backSlashIdx < 0) {
			int len = Math.toIntExact(end - start);
			byte[] out = new byte[len];
			MemorySegment.copy(segment, ValueLayout.JAVA_BYTE, start, out, 0, len);
			return new CompactString(out);
		}

		int byteLen = utf8LengthAfterUnescape(segment, start, end);
		byte[] out = new byte[byteLen];
		int written = writeUtf8AfterUnescape(segment, start, end, out, 0);
		assert written == byteLen;
		return new CompactString(out);
	}
	...
	private static int utf8LengthAfterUnescape(MemorySegment segment, long start, long end) {
		int len = 0;
		long i = start;
		...
		while (i < end) {
			...
			if (i + 1 >= end) {
				throw new IllegalArgumentException("Unescaped backslash in input segment");
			}

			byte esc = segment.get(ValueLayout.JAVA_BYTE, i + 1);
			switch (esc) {
			case 't', 'r', 'n', '"', '\\' -> {
				len++;
				i += 2;
			...
			case 'u' -> {
				if (i + 5 >= end) {
					throw new IllegalArgumentException("Incomplete Unicode escape sequence in input segment");
				}
				int cp = parseHex4(segment, i + 2, end, 'u');
				len += utf8LengthCodePoint(cp);
				i += 6;
			}
			case 'U' -> {
				...
				int cp = parseHex8ToInt(segment, i + 2, end, 'U');
				len += utf8LengthCodePoint(cp);
				i += 10;
			}
			default -> throw new IllegalArgumentException("Unescaped backslash in input segment");
			}
		}
	...
	private static int writeUtf8AfterUnescape(MemorySegment segment, long start, long end, byte[] out, int off) {
		long i = start;
		while (i < end) {
			byte b = segment.get(ValueLayout.JAVA_BYTE, i);
			if (b != '\\') {
	...
	private static long indexOf(MemorySegment segment, long start, long end, byte value) {
	...
	private static int parseHex4(MemorySegment segment, long pos, long end, char escapeType) {
		int h0 = hexValue(segment.get(ValueLayout.JAVA_BYTE, pos));
		int h1 = hexValue(segment.get(ValueLayout.JAVA_BYTE, pos + 1));
		int h2 = hexValue(segment.get(ValueLayout.JAVA_BYTE, pos + 2));
		int h3 = hexValue(segment.get(ValueLayout.JAVA_BYTE, pos + 3));
		if ((h0 | h1 | h2 | h3) < 0) {
			String xx = new String(new char[] { (char) segment.get(ValueLayout.JAVA_BYTE, pos),
					(char) segment.get(ValueLayout.JAVA_BYTE, pos + 1),
					(char) segment.get(ValueLayout.JAVA_BYTE, pos + 2),
					(char) segment.get(ValueLayout.JAVA_BYTE, pos + 3) });
			throw new IllegalArgumentException(
					"Illegal Unicode escape sequence '\\" + escapeType + xx + "' in input segment");
		}
	...
	private static int parseHex8ToInt(MemorySegment segment, long pos, long end, char escapeType) {
		long value = 0;
		for (int i = 0; i < 8; i++) {
			int hv = hexValue(segment.get(ValueLayout.JAVA_BYTE, pos + i));
			if (hv < 0) {
				StringBuilder sb = new StringBuilder(8);
				for (int j = 0; j < 8; j++) {
					sb.append((char) segment.get(ValueLayout.JAVA_BYTE, pos + j));
				}
				throw new IllegalArgumentException(
						"Illegal Unicode escape sequence '\\" + escapeType + sb + "' in input segment");
			}
			value = (value << 4) | hv;
		}
