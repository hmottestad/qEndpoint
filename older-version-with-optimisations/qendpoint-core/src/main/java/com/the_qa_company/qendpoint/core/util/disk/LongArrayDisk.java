...
public class LongArrayDisk implements Closeable, LongArray {
	...
	private static final int LOG2_MAPPING_BYTES = 30; // 1 << 30
	private static final int LOG2_LONG_BYTES = 3; // 8 bytes
	private static final int LOG2_LONGS_PER_MAP = LOG2_MAPPING_BYTES - LOG2_LONG_BYTES; // 27
	private static final long LONG_INDEX_MASK = (1L << LOG2_LONGS_PER_MAP) - 1; // 0x7FFFFFF

	@Override
	public long get(long index) {
		// keep it branchless + let JIT keep 'mappings' in a register
		final CloseMappedByteBuffer[] ms = this.mappings;
		if (ms.length == 1) {
			// index*8 is guaranteed to fit in int here because capacity <=
			// 1<<30
			return ms[0].getLong((int) (index << 3));
		}
		final int block = (int) (index >>> LOG2_LONGS_PER_MAP);
		final int offset = (int) ((index & LONG_INDEX_MASK) << LOG2_LONG_BYTES);

		return ms[block].getLong(offset);
	}
